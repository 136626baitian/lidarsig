...
%% 海盐气溶胶固定参数(不同气溶胶需要更改参数)
num_segments = 3;
densities = [2200, 1500, 1500]; % kg/m³
refractive_indices = [1.50 + 0.0001i, 1.450 + 0.001i, 1.35 + 0.001i]; %复折射率
geometric_stds = [1.6, 1.8, 2.0];  %几何标准差
median_diameters = [0.2, 2, 15]; % 中值直径
...
for i_plev = 1:n_plev
     i_time = 5;    %这里计算12点气溶胶光学性质
        try
            ...
            % 读取混合比（索引固定为第一个经纬度）
            aermr01 = ncread(ncfile, 'aermr01', [i_lon, i_lat, i_plev, i_time], [1,1,1,1]);
            aermr02 = ncread(ncfile, 'aermr02', [i_lon, i_lat, i_plev, i_time], [1,1,1,1]);
            aermr03 = ncread(ncfile, 'aermr03', [i_lon, i_lat, i_plev, i_time], [1,1,1,1]);

            % 计算光学参数
            mixing_ratios = [aermr01, aermr02, aermr03];
...
full_path = fullfile(savePath, 'seasalt.txt');
