...
num_segments = 1;
densities = [1500]; % kg/m³
refractive_indices = [1.8 + 2.0i]; %复折射率
geometric_stds = [1.8];  %几何标准差
median_diameters = [0.2]; % 峰值半径

% 粒径分布定义
r1 = 10.^(linspace(log10(0.03), log10(0.5), 100)); %积聚模态
particle_sizes = {r1};
...
for i_plev = 1:n_plev
     i_time = 5;    %这里计算12点气溶胶光学性质
        try
            ...
            % 读取混合比（索引固定为第一个经纬度）
            aermr09 = ncread(ncfile, 'aermr09', [i_lon, i_lat, i_plev, i_time], [1,1,1,1]);
            % 计算光学参数
            mixing_ratios = [aermr07];
